...
const Roster = () => {
  ...
  return (
    <div className="mx-auto flex max-w-6xl flex-col gap-6 px-4 py-8 pb-28 sm:py-12">
      ...
      <div className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-card dark:border-slate-800 dark:bg-slate-900">
        <div className="flex items-start justify-between gap-3">
          <div className="space-y-1">
            <p className="text-base font-semibold text-slate-900 dark:text-slate-50">Current user</p>
            <p className="text-sm text-slate-600 dark:text-slate-300">
              Demo-only switch (no authentication).
            </p>
          </div>
          <Link
            to="/staff"
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-0.5">
            <p className="text-base font-semibold text-slate-900 dark:text-slate-50">{activeUser?.name || '—'}</p>
            <p className="text-sm text-slate-600 dark:text-slate-300">{activeUser?.email || ''}</p>
          </div>
...
const UserDropdown = ({ value, users, onChange }) => {
  ...
  const selected = users.find((user) => user.id === value) || users[0]
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      >
        <span className="truncate">{selected?.name?.split(' ')[0] || 'User'}</span>
        <span className="text-slate-500 dark:text-slate-300">▾</span>
      </button>

      {open ? (
        <div className="absolute right-0 z-30 mt-2 w-56 overflow-hidden rounded-2xl border border-slate-200 bg-white p-1 shadow-card dark:border-slate-700 dark:bg-slate-900">
          {users.map((user) => {
            ...
            return (
              ...
              >
                ...
                <span className="ml-2 shrink-0 text-xs opacity-80">{user.role}</span>
              </button>
            )
          })}
        </div>
